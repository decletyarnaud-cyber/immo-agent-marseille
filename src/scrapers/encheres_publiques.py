...
class EncherePubliquesScraper(BaseScraper):
    ...
    # Cities to monitor with their URL slugs
    CITIES = {
        "marseille-13": "Marseille",
        "aix-en-provence-13": "Aix-en-Provence",
        "toulon-83": "Toulon",
        "aubagne-13": "Aubagne",
        "martigues-13": "Martigues",
        "la-ciotat-13": "La Ciotat",
        "hyeres-83": "Hyères",
        "la-seyne-sur-mer-83": "La Seyne-sur-Mer",
    }
    ...
    def scrape_all_cities(self, max_pages: int = 10) -> List[Auction]:
        """Scrape auctions from departments 13 (Bouches-du-Rhône) and 83 (Var)"""
        all_auctions = []
        seen_urls = set()

        # Scan all listings and filter for departments 13/83
        logger.info(f"[EnchèresPubliques] Scanning all listings for departments 13/83...")
        ...
        for page in range(1, max_pages + 1):
            ...
            # Filter for departments 13 and 83
            local_auctions = [
                a for a in auctions
                if "-13/" in a.get("url", "") or "-83/" in a.get("url", "")
            ]
            ...
            logger.debug(f"[EnchèresPubliques] Page {page}: {len(auctions)} total, {len(local_auctions)} in 13/83")

            # Stop if no more pages
            if len(auctions) < 10:
                break

        logger.info(f"[EnchèresPubliques] Total: {len(all_auctions)} auctions in 13/83")
        return all_auctions

...
def main():
    ...
    st.markdown('<p class="sub-header">Enchères Judiciaires Immobilières • Marseille • Aix-en-Provence • Toulon</p>', unsafe_allow_html=True)
...
def show_interactive_map(db: Database):
    """Interactive map showing all auctions with markers"""
    import folium
    from streamlit_folium import st_folium
    from folium.plugins import MarkerCluster

    st.subheader("🗺️ Carte des Enchères")

    auctions = db.get_all_auctions()

    if not auctions:
        st.info("Aucune enchère à afficher sur la carte")
        return
    ...
    with col1:
        filter_type = st.selectbox(
            "Type de bien",
            ["Tous", "Appartement", "Maison", "Local commercial", "Terrain", "Parking"],
            key="map_filter_type"
        )
    with col2:
        filter_score = st.slider("Score minimum", 0, 100, 0, key="map_filter_score")
    with col3:
    ...
    for auction in auctions:
        ...
        if filter_type != "Tous":
            ...
            if auction.type_bien and str(auction.type_bien.name) != type_map.get(filter_type, ""):
                continue

        # Score filter
        if auction.score_opportunite and auction.score_opportunite < filter_score:
            continue

        filtered_auctions.append(auction)

    st.caption(f"{len(filtered_auctions)} enchères affichées sur {len(auctions)} total")

    # Center map on Marseille region
    center_lat, center_lon = 43.2965, 5.3698  # Marseille center
    ...
    for auction in filtered_auctions:
        ...
        if lat and lon:
            ...
            type_icons = {
                ...
                "MAISON": "home",
                "LOCAL_COMMERCIAL": "briefcase",
                "TERRAIN": "tree",
                "PARKING": "car"
            }
            type_name = auction.type_bien.name if auction.type_bien else "APPARTEMENT"
            icon_name = type_icons.get(type_name, "home")

            # Format price
            prix = f"{auction.mise_a_prix:,.0f}€".replace(",", " ") if auction.mise_a_prix else "N/A"
            surface = f"{auction.surface}m²" if auction.surface else "?"
            decote = f"{auction.decote_pourcentage:.0f}%" if auction.decote_pourcentage else "N/A"
            stars = "★" * (int(score) // 20) if score and score > 0 else "☆"
            decote_color = "#2e7d32" if auction.decote_pourcentage and auction.decote_pourcentage > 30 else "#666"
            annonce_link = f'<a href="{auction.url}" target="_blank" style="display: inline-block; padding: 6px 12px; background: #1e3a5f; color: white; text-decoration: none; border-radius: 4px; font-size: 12px; margin-left: 5px;">🔗 Annonce</a>' if auction.url else ''
            ...
            <div style="font-family: Arial, sans-serif; min-width: 250px; max-width: 300px;">
                <h4 style="margin: 0 0 8px 0; color: #1e3a5f;">{auction.ville or 'N/A'}</h4>
                <p style="margin: 4px 0; font-size: 12px; color: #666;">{auction.adresse or 'Adresse non précisée'}</p>
                <hr style="margin: 8px 0; border: none; border-top: 1px solid #ddd;">
                <table style="width: 100%; font-size: 13px;">
                    <tr><td><b>Type:</b></td><td>{type_name.replace('_', ' ').title()}</td></tr>
                    <tr><td><b>Surface:</b></td><td>{surface}</td></tr>
                    <tr><td><b>Prix:</b></td><td style="color: #2e7d32; font-weight: bold;">{prix}</td></tr>
                    <tr><td><b>Décote:</b></td><td style="color: {decote_color};">{decote}</td></tr>
                    <tr><td><b>Score:</b></td><td>{stars} ({score}/100)</td></tr>
                </table>
                <div style="margin-top: 10px; text-align: center;">
                    <a href="{google_maps_url}" target="_blank"
                       style="display: inline-block; padding: 6px 12px; background: #4285f4; color: white;
            ...
            marker = folium.Marker(
                ...
                tooltip=f"{auction.ville} - {prix} ({decote})",
    ...
    # Legend
    st.markdown("""
    <div style="background: rgba(30, 58, 95, 0.3); padding: 10px; border-radius: 8px; margin-top: 10px;">
        <b>Légende:</b>
        <span style="color: #2e7d32;">🟢 Score ≥75</span> |
        <span style="color: #ff9800;">🟠 Score 50-74</span> |
        <span style="color: #2196f3;">🔵 Score 25-49</span> |
        <span style="color: #9e9e9e;">⚫ Score <25</span>
    </div>
    """, unsafe_allow_html=True)

    st.caption(f"📍 {geocoded_count} adresses géolocalisées sur {len(filtered_auctions)} enchères")
